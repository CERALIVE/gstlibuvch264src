...
- Do NOT raise a `QUIRK_MAX_PIXEL_RATE` cap on the strength of a descriptor, a datasheet, or a successful `uvc_get_stream_ctrl_format_size()`. Only frames that actually ADVANCE on real hardware justify a higher cap; the cap is deliberately parked at the last CONFIRMED-GOOD rate, not the last known-bad one, because a cap set too low only costs resolution while a cap set too high costs the whole stream. (For the Osmo Pocket 3 the recorded zero-frame 4K@60 behaviour did NOT reproduce on 2026-07-30 — it delivered sustained SPS-verified 4K. The cap stays until an owner decides; see `camera-compat.md` §2 Step 5.)
